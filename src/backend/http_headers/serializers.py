...
class HttpHeaderSerializer(ModelSerializer):
    ...
    def validate(self, attrs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if attrs.get("target"):
            attrs["user"] = None
        if (
            attrs.get("user") is not None and attrs.get("user") != self.context.get("request").user
        ) or (
...
class SimpleHttpHeaderSerializer(ModelSerializer):
    ...
    def update(self, instance: HttpHeader, validated_data: Dict[str, Any]) -> HttpHeader:
        if (instance.user is not None and instance.user != self.context.get("request").user) or (instance.user is None and instance.target is None and not IsAdmin().has_permission(self.context.get("request"), None)):
            raise PermissionDenied()
