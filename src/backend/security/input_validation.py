...
SECRET_REGEX = (
    r"[\w\./\-=\+,:<>¿?¡!#&$()@%\[\]\{\}\*]{1,500}"  # Regex for credentials validation
)
...
def validate_text_value(value: str, regex: str) -> None:
    ...
    if not bool(re.fullmatch(regex, value)):
        logger.warning(
            f"[Security] Invalid text value that doesn't match the regex {regex}"
        )
...
def validate_number_value(value: int, min: int, max: int) -> None:
    ...
    if value < min or value > max:
        logger.warning(
            f"[Security] Invalid number value {value} that is not in the range {min} - {max}"
        )
...
def validate_secret(value: str) -> None:
    """Validate if secret is valid based on regex.
    ...
    Raises:
        ValidationError: Raised if value doesn't match the expected regex
    """
    validate_text_value(value, SECRET_REGEX)
