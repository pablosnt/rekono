...
class DefectDojoIntegrationTest(RekonoTest):
    ...
    def test_project_sync(self) -> None:
        ApiTestCase(["admin1"], "post", 201, sync, {"id": 1, **sync}, "/api/defect-dojo/sync/").test_case()
        self._setup_findings(self.execution3)
        self.execution3.output_file = self.data_dir / "reports" / "nmap" / "enumeration-vulners.xml"
    ...
    def test_target_sync(self) -> None:
        ...
        ApiTestCase(["auditor1"], "post", 201, sync, {"id": 1, **sync}, "/api/defect-dojo/sync/").test_case()
        self.assertFalse(DefectDojoTargetSync.objects.filter(target=self.execution1.task.target).exists())
        self._setup_findings(self.execution1)
        integration = DefectDojo()
        integration.process_findings(self.execution1, self.findings)
        self.assertTrue(DefectDojoTargetSync.objects.filter(target=self.execution1.task.target).exists())
        ...
        self.assertEqual(
            1,
            DefectDojoTargetSync.objects.filter(target=self.execution1.task.target).count(),
        )
        self.assertIsNone(self.execution3.defectdojo_test_id)
        for finding in self.findings:
            self.assertEqual(1, finding.defectdojo_id)
