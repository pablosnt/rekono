...
class Report(BaseModel):
    project = models.ForeignKey(
        Project, related_name="reports", on_delete=models.CASCADE, blank=True, null=True
    )
    target = models.ForeignKey(
        Target, related_name="reports", on_delete=models.CASCADE, blank=True, null=True
    )
    task = models.ForeignKey(
        Task, related_name="reports", on_delete=models.CASCADE, blank=True, null=True
    )
    status = models.TextField(
        max_length=7, choices=ReportStatus.choices, default=ReportStatus.PENDING
    )
