...
class Sshaudit(BaseParser):
    ...
    def _parse_standard_output(self) -> None:
        algorithms: dict[str, list[str]] = {k: [] for k in self.cryptography_types.keys()}
        technology = None
        ...
        for line in self.output.split("\n"):  # Get output by lines
            ...
            if "(gen) software: " in data:
                ...
                vulnerabilities_to_create.append((aux[1].split(") ", 1)[1].split(" [", 1)[0].capitalize(), aux[0]))
            else:
                for cryptography_type in algorithms.keys():
                    if f"({cryptography_type}) " in data and "[fail]" in data:
                        algorithm = data.split(f"({cryptography_type}) ", 1)[1].split(" ", 1)[0]
        ...
        for name, cve in vulnerabilities_to_create:
            self.create_finding(Vulnerability, technology=technology, name=name, cve=cve)
