<template>
    <v-form v-model="valid" @submit.prevent="submit()">
        ...
        <v-file-input v-if="!edit"
            ...
            :prepend-icon="null"
            accept="text/plain"
            :rules="[f => !!f || 'File is required', f => !f.length || f[0].size <= (maxSize * 1000000) || `File size must be less than ${maxSize} MB`]"
            validate-on="blur"
        />
...
<script setup lang="ts">
    ...
    useApi('/api/settings/', true, 'Settings').get(1).then((data) => maxSize.value = data.max_uploaded_file_mb)
    ...
    function submit() {
        if (valid.value) {
            ...
            request(body, props.edit?.id).then((data) => { emit('completed', data) }).catch(() => { emit('loading', false) })   
        }
    }
</script>