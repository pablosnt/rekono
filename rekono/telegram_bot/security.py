...
def get_chat(update: Update, auditor: bool = True) -> Union[TelegramChat, None]:
    '''Get Telegram chat entity after checking linked account and user roles.
    ...
    if update.effective_chat:                                                   # Chat Id from the update
        chat = TelegramChat.objects.filter(chat_id=update.effective_chat.id).first()    # Get chat entity
